...
class ShellyDevice(object):
	...
	def __init__(self, bus_type=None, serial=None, server=None):
		...
		self._rpc_with_reconnect_lock = asyncio.Lock() # Protects the rpc_call_with_reconnect from being called concurrently
	...
	async def connect(self):
		...
		try:
			...
			if not await self.set_device_info():
	...
	async def _reinit_channel_and_handlers(self, ch):
		...
		if cap_changed:
			...
			channel_obj = self._channels[ch].get("channel")
			await channel_obj.reinit(self.rpc_call_with_reconnect, partial(self.restart_channel, ch))
	...
	async def set_device_info(self):
		...
		if self.is_supported():
			...
			# Device is not supported, clear channel info.
			self._channel_info = {}
			# Device capability probe was successful, but the device is not supported.
			# The capabilities will indicate it is not supported.
			return True
	...
	async def start_channel(self, channel):
		...
		async with self._device_lock:
			if not self.is_connected:
				# Not connected or in the process of reconnecting.
				return False
			if channel in self._channels:
				# Channel is already active; avoid creating/registering a duplicate service.
				return True
			if channel in self._init_channel_tasks and self._init_channel_tasks[channel] is not None:
				# Should not happen.
				...
				for cap in self._capabilities:
					...
					if create_new:
						handler = await shelly_handlers.ShellyHandler.create(
							cap,
							rpc_callback=self.rpc_call_with_reconnect,
	...
	# RPC call routine that will wait on the reconnect task when the RPC call fails, and will retry the RPC call after reconnection.
	async def rpc_call_with_reconnect(self, method, params=None):
		# Serialize RPC calls from the handlers
		async with self._rpc_with_reconnect_lock:
			resp = None
			if not self.is_connected:
				return None
			while True:
