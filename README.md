...
# Meter polling
By default, the native driver waits for Shelly WebSocket status notifications for instantaneous meter values. To request fresher instantaneous meter status, set `/Settings/Shelly/MeterPollInterval` to a polling interval in seconds, or write the same value to `/MeterPollInterval` on the `com.victronenergy.shelly` service.

The default value is `0`, which disables polling and preserves the notification-only behavior. Nonzero values use a minimum interval of 1 second. Polling only requests instantaneous meter status for active EM, EM1 and PM1 meter channels; energy counters continue to update from their existing status-notification path.
