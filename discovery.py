...
class ConnectMeta:
	...
	# Immediately retry connecting, set the deadline to the retry window from now and clear the attempts.
...
class ShellyDeviceCache:
	...
	async def add(self, endpoint: ShellyEndpoint, start_connect = True) -> None:
	...
	async def promote_endpoint_to_serial(self, host: str, serial: str) -> ShellyEndpointState | None:
		...
		try:
			...
			if ep_state.connect.due(now):
				...
				async with cache_lock:
					...
					# Reachable and supported device -> mark as supported and reset retry state
				...
				if promote_serial and result.info is not None:
					...
					promoted_state = await self.shelly_device_cache.promote_endpoint_to_serial(ep_state.endpoint.host, serial)
					if promoted_state is not None:
						ep_state = promoted_state
				if remove_entry:
...
class ShellyManager(object):
	...
	def clear_discovered_device_paths(self, serial):
		with self.service as s:
			...
			s['/Devices/{}/LastSeen'.format(serial)] = None
			s['/Devices/{}/Reachable'.format(serial)] = None
	...
	async def _on_enabled_changed(self, serial, channel, item, value):
		...
		if len([x for x in self._channel_op_queue._queue if x.serial == serial and x.channel == channel]) >= 4:
			return

		item.set_local_value(value)
		operation = ChannelOperation(serial, channel, value, item)
